...
* **Input Protection:** 100Ω series resistors and low-capacitance `PRTR5V0U2X` TVS diode clamps protect against static and overvoltage.
* **Switchable I2C Pull-Ups:** 2-position DIP switch enables onboard 2.2kΩ pull-up resistors on Channel 0 (SDA) and Channel 1 (SCL).
...
1. **Signal Conditioning:** External probe signals (1.8V to 5.5V) pass through current-limiting resistors and ESD TVS diodes into the `74LVC245A` buffer, which outputs clean 3.3V logic signals.
2. **Hardware Sampling:** The RP2040 PIO state machine reads all 8 GPIO pins (`GPIO0` to `GPIO7`) in a single clock cycle, packing four 8-bit samples into 32-bit words.
...
4. **Dual Output Processing:**
   ...
   * **Core 1** reads the SRAM buffer, renders the 8 waveform traces on the 2.4-inch color LCD at ~33 FPS, and responds to 5-way joystick inputs.
...
| **UI Navigation** | `NAV_LEFT` | **GPIO18** (Pin 29) | Joystick LEFT | `PIN_NAV_LEFT` | Timebase Zoom Out (1µs to 100ms) |
| **UI Navigation** | `NAV_RIGHT` | **GPIO19** (Pin 30) | Joystick RIGHT | `PIN_NAV_RIGHT` | Timebase Zoom In (100ms to 1µs) |
...
3. Attach probe channels `CH0` to `CH7` to the target digital lines.
4. If testing an un-terminated I2C bus, turn **DIP Switch 1 & 2** `ON` to activate the 2.2kΩ pull-up resistors.
5. Use the **5-Way Joystick**:
   * **Left / Right:** Adjust horizontal timebase (1µs/div to 100ms/div).
